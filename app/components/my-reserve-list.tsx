    ...
    <div>
        {reservation.map((item) =>(
            <div className="bg-white shadow pb-4 mb-4 md:pb-0 relative" key={item.id}>
                ...
                <div className="flex flex-col mb-4 items-start bg-white rounded-sm md:flex-row md:w-full">
                    ...
                    <div className="flex items-center gap-1 mb-3 font-normal text-gray-700 w-full">
                        <div className="w-full">
                            <div className="flex items-center justify-between text-sm font-medium text-gray-900 tuncate">
                                <span>Price</span>
                ...
                <div className="flex item-end justify-end absolute inset-4">
                    {item.Payment?.status === "unpaid" ? (
                        ...
                        <Link href={`/myreservation/${item.id}`} className='px-5 py-1 bg-orange-400 text-white rounded-md hover:bg-orange-500'>View Details</Link>
                    )}
                </div>
            </div>
        ))}
